...
$ldapuser      = 'GLOCAD\alberto_c';  //GLOCAD\USUARIO
$usuario_final=$_SESSION['usuario'];
...
if($ldapconn) {
    ...
    if ($ldapbind) {
        ...
        for ($i=0; $i<$data["count"]; $i++) {
            //echo "dn is: ". $data[$i]["dn"] ."<br />";
            if(isset($data[$i]["wwwhomepage"][0])) {
                $powerbi=utf8_encode($data[$i]["wwwhomepage"][0]);
            } else {
                $powerbi="";
            }
            //$powerbi=utf8_encode($data[$i]["wwwhomepage"][0]);
        ...
        for ($i=0; $i<$data["count"]; $i++) {
            //echo "dn is: ". $data[$i]["dn"] ."<br />";
            $alias=utf8_encode($data[$i]["samaccountname"][0]);
            //echo "User: ". $data[$i]["cn"][0] ."<br />           
        }
         ...
         for ($i=0; $i<$data["count"]; $i++) {
            ...
            //echo "User: ". $data[$i]["cn"][0] ."<br />  
              ...
              if(isset($data[$i]["initials"][0])) {
                $PBI=utf8_encode($data[$i]["initials"][0]);
            } else {
                //echo "Departamento: None<br /><br />";
                $PBI="";
            }         
